...
struct TurnView: View {
        ...
        return TurnConversationContainerView(
            ...
            composer: AnyView(
                TurnComposerHostView(
                    ...
                    isReconnectAvailable: codex.hasSavedBridgePairing,
                    isReconnectInFlight: isReconnectInFlight || codex.isConnecting,
    ...
    private func handleReconnect() {
        guard codex.hasSavedBridgePairing, !isReconnectInFlight, !codex.isConnecting else {
            return
        }
        ...
        Task { @MainActor in
            ...
            do {
                await reconnectCoordinator.stopAutoReconnectForManualScan(codex: codex)
    ...
    private var connectionStatusMessage: String {
        ...
        if isReconnectInFlight || codex.isConnecting {
        ...
        if let errorMessage = codex.lastErrorMessage?
    ...
    private var timelineErrorMessage: String? {
        guard let errorMessage = codex.lastErrorMessage?
            .trimmingCharacters(in: .whitespacesAndNewlines),
              !errorMessage.isEmpty else {
            return nil
        }

        if codex.isConnected, codex.isTransientConnectionStatusMessage(errorMessage) {
            return nil
        }
