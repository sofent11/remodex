...
private let coderoverMarkdownGitHubSecondaryBackground = DynamicColor(
    light: Color(red: 247 / 255, green: 247 / 255, blue: 249 / 255),
    dark: Color(red: 37 / 255, green: 38 / 255, blue: 42 / 255)
)

private let coderoverMarkdownGitHubLink = DynamicColor(
    ...
    private var inlineStyle: InlineStyle {
        InlineStyle()
            .code(
                .font(AppFont.mono(.body)),
                .fontScale(0.85),
                .backgroundColor(coderoverMarkdownGitHubSecondaryBackground)
            )
            .strong(.fontWeight(.semibold))
            .link(.foregroundColor(coderoverMarkdownGitHubLink))
    }
...
private struct CodeRoverMarkdownCodeBlockStyle: StructuredText.CodeBlockStyle {
    func makeBody(configuration: Configuration) -> some View {
        ...
        .background(coderoverMarkdownGitHubSecondaryBackground)
...
private enum AttachmentPreviewImageResolver {
    ...
    static func resolve(_ attachment: ImageAttachment) -> UIImage? {
...
private struct CommandExecutionStatusCard: View {
    ...
    private var detailModel: CommandExecutionDetails? {
        ...
        return coderover.commandExecutionDetailsByItemID[itemId]
    }
}
